-- 1. Helper function to extract Firebase UID from the Supabase JWT
-- In a generic JWT from Firebase, the user ID is in the 'sub' claim.
...
CREATE POLICY "Users can manage own saved routes" ON public.saved_routes FOR ALL USING (user_id = public.firebase_uid());

-- Trips policies
CREATE POLICY "Users can read own trips" ON public.trips FOR SELECT USING (user_id = public.firebase_uid());
CREATE POLICY "Users can insert their own trips" ON public.trips FOR INSERT WITH CHECK (user_id = public.firebase_uid());
CREATE POLICY "Users can update their own trips" ON public.trips FOR UPDATE USING (user_id = public.firebase_uid());
